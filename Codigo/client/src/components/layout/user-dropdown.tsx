...
export function UserDropdown() {
  ...
  const displayName = profile?.nome?.trim() || user.email.split('@')[0]
  const avatarUrl = profile?.imageUrl?.trim() || ''
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <div className="relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-md bg-primary-600 text-white">
          {avatarUrl ? (
            <Image
              src={avatarUrl}
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-xl border border-border bg-surface p-2 shadow-2xl ring-1 ring-black/5 focus:outline-none z-50">
          <div className="px-3 py-2 border-b border-border/50 mb-1">
            ...
            <p className="text-sm font-medium text-text-primary truncate">{accountEmail}</p>
          </div>
